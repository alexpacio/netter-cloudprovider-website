function stickerElementChangeLanguage(language:string) {
    const navItems:HTMLCollectionOf<HTMLElement> =document.getElementById('navbar-container').getElementsByTagName('a');
    const footerItems:HTMLCollectionOf<HTMLAnchorElement> = document.getElementById('footer-container').getElementsByTagName('a');
    if (language === 'EN') {
        ...
        for (let i:number = 0; i < footerItems.length; i++) {
